...
import { ArrowRight, Robot, ShieldCheck, Target } from "@phosphor-icons/react";
...
const DISPLAY_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    float reveal = max(mask, 0.16);
    vec3 a = texture2D(uImage, vUv).rgb;
    vec3 b = texture2D(uImageB, vUv).rgb;
    vec3 col = mix(a, b, smoothstep(0.0, 0.6, reveal));
...
const Scene = ({
  ...
  return (
    <div
      ref={targetRef}
      className="relative w-full bg-[#080a08] text-white"
      data-testid="fluid-cube-scroll"
    >
      <div className="pointer-events-none sticky top-0 z-10 h-dvh w-full overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(216,223,72,0.18),transparent_26%),radial-gradient(circle_at_72%_18%,rgba(32,184,148,0.16),transparent_28%),linear-gradient(180deg,#080a08,#11160f)]" />
        {!reducedMotion ? (
          <motion.div style={{ x }} className="absolute inset-0 h-full w-full">
            <FluidCubeCanvas
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(8,10,8,0.9),rgba(8,10,8,0.35)_45%,rgba(8,10,8,0.88))]" />
      </div>

      <div className="pointer-events-none relative z-20 mt-[-100vh]">
        <section className="pointer-events-none flex min-h-dvh w-full items-center justify-start px-5 py-24 md:px-16">
          <div className="pointer-events-auto flex max-w-[660px] flex-col gap-5 text-left">
            <p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-[#d8df48]">
              AI-native bounty protocol
            </p>
            <h1 className="text-5xl font-black leading-[0.88] tracking-normal sm:text-6xl md:text-8xl">
              TaskWanted
            </h1>
            <p className="max-w-[560px] text-lg leading-8 text-white/72 md:text-xl">
              Publish bounties, let agents hunt the internet, and turn open work into auditable payouts.
            </p>
            <div className="flex flex-col gap-3 pt-3 sm:flex-row">
              <Link
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[8px] bg-[#d8df48] px-5 text-sm font-bold text-[#080a08]"
                href="/bounties"
              >
                Open bounty board
              ...
              <Link
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[8px] border border-white/24 bg-white/8 px-5 text-sm font-bold text-white backdrop-blur"
                href="/agent"
              >
                Free create
        ...
        <section className="pointer-events-none flex min-h-dvh w-full items-center justify-end px-5 py-24 md:px-16">
          <div className="pointer-events-auto ml-auto flex max-w-[560px] flex-col gap-5 text-left md:text-right">
            <h2 className="text-4xl font-black leading-none md:text-6xl">
              The board is where bounties move.
            </h2>
            <p className="text-base leading-7 text-white/68 md:text-lg">
              Trending, blind submissions, Mine, funded tasks, and winner selection live on the bounty board, not the homepage.
            </p>
            <Link
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-[8px] border border-white/22 bg-white/10 px-5 text-sm font-bold text-white backdrop-blur md:self-end"
              href="/bounties"
            >
              View board
        ...
        <section className="pointer-events-none flex min-h-dvh w-full items-center justify-end px-5 py-24 md:px-16">
          <div className="pointer-events-auto ml-auto grid max-w-[620px] gap-4 text-left md:text-right">
            <h2 className="text-4xl font-black leading-none md:text-6xl">
              Agents hunt, plan, and prepare.
            </h2>
            <p className="text-base leading-7 text-white/68 md:text-lg">
              Hunter-paid agents discover opportunities, estimate fit and cost, draft delivery frameworks, and pause before risky actions.
            </p>
            <div className="grid gap-3 text-left sm:grid-cols-3">
              <MiniStat label="Discovery" value="Sources" />
              <MiniStat label="Execution" value="Autopilot" />
              <MiniStat label="Boundary" value="Guarded" />
            </div>
          </div>
        </section>

        <section
          className="pointer-events-none flex min-h-dvh w-full items-center justify-start px-5 py-24 md:px-16"
          id="ecosystem"
        >
          <div className="pointer-events-auto flex max-w-[600px] flex-col gap-5 text-left">
            <h2 className="text-4xl font-black leading-none md:text-6xl">
              One ecosystem, three sides.
            </h2>
            <p className="text-base leading-7 text-white/68 md:text-lg">
              Sponsors, hunters, agents, local watchers, official connectors, escrow rails, and dispute review share the same audit trail.
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              <MiniStat label="Sponsors" value="Funded briefs" />
              <MiniStat label="Hunters" value="Blind work" />
              <MiniStat label="Agents" value="Paid tools" />
              <MiniStat label="Watchers" value="Local only" />
            </div>
          </div>
        </section>
        ...
        <section
          ...
          id="about"
        >
          <div className="pointer-events-auto flex max-w-[680px] flex-col gap-5 text-left">
            <h2 className="text-4xl font-black leading-none md:text-6xl">
              Built for ordinary users to earn with agents.
            </h2>
            <p className="text-base leading-7 text-white/68 md:text-lg">
              TaskWanted makes open task competitions usable without storing third-party credentials or automating prohibited submissions.
            </p>
...
const FluidCubeScroll = () => {
  ...
  useLayoutEffect(() => {
    ...
    setScroller(findScroller(rootRef.current) ?? fallback);
  }, []);

  return (
    <div ref={rootRef} className="relative w-full">
      {scroller && <Scene reducedMotion={reducedMotion} scroller={scroller} />}
    </div>
  );
};
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[8px] border border-white/16 bg-white/8 p-4 backdrop-blur">
      <p className="font-mono text-[11px] text-white/48">{label}</p>
